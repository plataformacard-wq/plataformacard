...
const REPLACEMENTS = [
  ...
  // 3. Garantir que as sessões estiquem até a margem do navegador (PanelLayout) e não fiquem centralizadas e pequenas
  {
  ...
  // 4. Substituir arredondamentos hardcoded arbitrários (ex: rounded-[32px], rounded-[40px]) para o padrão de containers rounded-[27px]
  {
    regex: /(?<![a-zA-Z0-9-])rounded-\[(?:3[0-9]|4[0-9]|5[0-9])px\](?![a-zA-Z0-9-])/g,
    replace: 'rounded-[27px]',
    desc: "Padronizando arredondamentos customizados extremos para rounded-[27px]"
  }
];
