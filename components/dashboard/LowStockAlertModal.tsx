      ...
      {isOpen && (
        <motion.div key="alert-modal" className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative flex flex-col w-full max-w-3xl max-h-[85vh] overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 shadow-2xl border border-red-500/20">
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-white dark:bg-zinc-900">
               {products.length > 0 ? (
                       ...
                       <div className="flex items-center gap-3">
                         {editingId === p.id ? (
                           <div className="flex items-center gap-2">
                             <input type="number" min="0" value={editValue} onChange={e => setEditValue(e.target.value)} className="w-20 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-2 py-1.5 text-sm outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500" autoFocus />
                           ...
                           <>
                             ...
                             <button onClick={() => handleEdit(p)} className="text-xs font-medium bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm px-3 py-1.5 rounded-lg text-zinc-700 dark:text-zinc-300 hover:border-red-500 hover:text-red-600 dark:hover:text-red-400 transition ml-2">
                               Ajustar
                             </button>
                           </>
                         )}
